// 파일 관련
var fileIndex = document.querySelectorAll('.fileBox').length; // 0개면 다음에 넣을 파일의 인덱스는 0
...
/* 컨트롤러로 전송 */
function submitForm(tradingIdx) {
    // 데이터 가져오기 (첨부파일 제외)
    ...
    // 살아남은 기존 파일들
    var oldFiles = document.querySelectorAll('.oldFile')
    for (var i = 0; i < oldFiles.length; i++) {
        formData.append("oldFileIdxArr", oldFiles[i].getAttribute("data-file_idx"));
    }
    // 새로 넣은 파일
    ...
    // 살아남은 기존 파일들
    for (var i = 0; i <  oldFiles.length; i++) {
        console.log('(old) idx of oldFiles[' + i + ']: ' + oldFiles[i]);
    }
    // 새로 넣은 파일
    for (var i = 0; i < filesArr.length; i++) {
        if (!filesArr[i].is_delete) {
            console.log('(new) name of filesArr[' + i + ']: ' + filesArr[i].name);
        }
    }

    // 데이터 보내기
    fetch('http://localhost/trading/modify.do', {
        method: 'POST',
        body: formData,
        redirect: 'manual'
    })
